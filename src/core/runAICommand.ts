...
export async function runAICommand(options: RunAICommandOptions) {
  const { context, action, panelMode, loadingMessage } = options;
  ...
  try {
    ...
    vscode.window.setStatusBarMessage(
      `$(sync~spin) ${loadingMessage || "MundiCodeLens thinking..."}`,
      2000,
    );
    ...
    const formattedContext = formatPromptContext(projectContext);
    ...
    console.log("🚀 FINAL RESPONSE TO UI:", aiResponse);

    showPanel(context, aiResponse, selectedText, panelMode || action);
  } catch (error: any) {
